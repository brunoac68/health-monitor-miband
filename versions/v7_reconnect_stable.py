"""
v7_reconnect_stable.py

VERSÃO V7 — ESTÁVEL PARA BLEAK / BLUEZ
...
async def cleanup(client):
    for uuid in (UUID_AUTH, UUID_HR_MEAS):
        try:
...
async def monitor(client):
    ...
    try:
        # Pequena pausa para BLE estabilizar
        ...
        if not challenge:
            raise RuntimeError("Auth challenge não recebido")
        ...
        while True:
            ...
            if last_hr_time and now - last_hr_time > WATCHDOG_TIMEOUT:
                raise RuntimeError("Watchdog: sem HR por muito tempo")
