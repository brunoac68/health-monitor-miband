...
- Detecta carregamento mesmo no startup
- Detecta carregamento durante execução
- Registra eventos de carregamento/remoção no SQLite
Estados:
- IN_USE
- CHARGING
- REMOVED
...
def update_wearable_state(battery, hr_recent):
    ...
    if wearable_state == "IN_USE" and not hr_recent:
        set_state("REMOVED", f"[{now}] ❌ Pulseira fora do pulso")
